var express = require("express");
var fs = require("fs");
var bodyParser = require('body-parser');
var cors = require('cors');
var MongoClient = require('mongodb').MongoClient;
var url = "mongodb://localhost:27017/employeeDB";
var jwt = require("jsonwebtoken");
var fileUpload = require("express-fileupload");

var app = express();
app.use(bodyParser.urlencoded({ extended: true }));
app.use(bodyParser.json());
app.use(bodyParser.json({ type: 'application/*+json' }));
app.use(cors());
app.use(fileUpload({
    createParentPath: true
}));

const SECRET_KEY = "SECRET_KEY";

const generateJWTToken = (userData) => {
    return jwt.sign(userData, SECRET_KEY, { expiresIn: 3600 });
}

const verifyToken = (jwtToken) => {
    try {
        return jwt.verify(jwtToken, SECRET_KEY);
    } catch (e) {
        console.log('e:', e);
        return null;
    }
}

app.get("/verifyUser/:userName", async function (req, res) {

    var client = await MongoClient.connect(url);
    var dbo = client.db("employeeDB");

    var result = await dbo.collection("employee").find({ userName: req.params.userName }).toArray();
    if (result.length > 0)
        res.send({ Registered: true });
    else
        res.send({ Registered: false });
});

app.get("/getUsers", async function (req, res) {

    var client = await MongoClient.connect(url);
    var dbo = client.db("employeeDB");

    var result = await dbo.collection("employee").find({}).toArray();
    res.send(result);
    client.close();

});

app.get("/getUser/:username", async function (req, res) {

    var client = await MongoClient.connect(url);
    var dbo = client.db("employeeDB");
    var result = await dbo.collection("employee").findOne({ userName: req.params.username });
    if (result.length == 0)
        res.sendStatus(404);
    else {
        res.send(result);
    }

    client.close();

});

app.get("/getAduser/:username", async function (req, res) {

    var client = await MongoClient.connect(url);
    var dbo = client.db("employeeDB");
    var result = await dbo.collection("adUser").findOne({ userName: req.params.username });
    if (result.length == 0)
        res.sendStatus(404);
    else
        res.send(result);

    client.close();
});

app.get("/getReportingUsers", async function (req, res) {

    var client = await MongoClient.connect(url);
    var dbo = client.db("employeeDB");
    var result = await dbo.collection("adUser").find({}, { projection: { _id: 0, userName: 1 } }).toArray();
    res.send(result);
    if (result.length == 0)
        res.sendStatus(404);
    else
        res.send(result);

    client.close();
});

app.get("/getDepartments", async function (req, res) {
    res.setHeader("Content-Type", "application/json");
    res.setHeader("Access-Control-Allow-Origin", "*");

    var client = await MongoClient.connect(url);
    var dbo = client.db("employeeDB");
    var result = await dbo.collection("department").find({}).toArray();
    res.send(result);
    client.close();
});

app.get("/getDesignations", async function (req, res) {
    res.setHeader("Content-Type", "application/json");
    res.setHeader("Access-Control-Allow-Origin", "*");

    var client = await MongoClient.connect(url);
    var dbo = client.db("employeeDB");
    var result = await dbo.collection("designation").find({}).toArray();
    res.send(result);
    client.close();
});

app.get("/regions", async function (req, res) {
    res.setHeader("Content-Type", "application/json");
    res.setHeader("Access-Control-Allow-Origin", "*");

    var client = await MongoClient.connect(url);
    var dbo = client.db("employeeDB");
    var result = await dbo.collection("region").find({}).toArray();
    res.send(result);
    client.close();
});

app.post("/saveUser", async function (req, res) {
    res.setHeader("Content-Type", "application/json");
    res.setHeader("Access-Control-Allow-Origin", "*")
    var client = await MongoClient.connect(url);
    var dbo = client.db("employeeDB");
    var user = await dbo.collection("employee").findOne({ userName: req.body.userName });
    var len = await dbo.collection("employee").find({}).toArray().length;
    const defaultImage = "";

    if (!IsEmpty(user)) {
        req.body._id = user._id;
        req.body.photo = IsEmpty(user.photo) ? defaultImage : user.photo;
    }
    else {
        req.body._id = len + 1;
        req.body.photo = defaultImage;
    }

    await dbo.collection("employee").save(req.body);
    var token = generateJWTToken({ userName: req.body.userName });
    res.setHeader("x-access-token", token);
    var result = await dbo.collection("employee").findOne({ userName: req.body.userName });
    res.send(result);


    client.close();
});

app.post("/uploadPhoto", async function (req, res) {

    if (req.files) {
        let photo = req.files.photo;
        var userName = photo.name;
        var client = await MongoClient.connect(url);
        var dbo = client.db("employeeDB");
        var user = await dbo.collection("employee").findOne({ userName: userName });
        if (user) {
            user.photo = photo.data;
            await dbo.collection("employee").save(user);
            return res.send('{result:true}');
        }

    }

    res.send('{result:false}');

    client.close();
});


app.delete("/deleteUser/:username", async function (req, res) {
    var token = req.headers["x-access-token"];
    if (!token)
        return res.sendStatus(403);

    jwt.verify(token, SECRET_KEY, async function (err, decoded) {
        if (err)
            return res.sendStatus(403);

        if (decoded) {
            res.setHeader("Content-Type", "application/json");

            var client = await MongoClient.connect(url);
            var dbo = client.db("employeeDB");
            var user = await dbo.collection("employee").find({ userName: req.params.userName }).toArray();
            if (user.length == 0)
                res.sendStatus(404);
            else {
                await dbo.collection("employee").delete({ userName: req.params.username });

                var result = await dbo.collection("employee").find({}).toArray();
                res.send(result);
            }

            client.close();
        }
    });
});

app.post("/insertAdUser", async function (req, res) {
    res.setHeader("Content-Type", "application/json");

    var client = await MongoClient.connect(url);
    var dbo = client.db("employeeDB");
    await dbo.collection("adUser").insert(req.body);

    var result = await dbo.collection("adUser").find({}).toArray();
    res.send(result);
    client.close();
});

const IsEmpty = (obj) => {
    return obj == null || obj == undefined || obj == "";
};

var server = app.listen(8081, function () {
    var host = server.address().address
    var port = server.address().port;
    console.log("app listening at http://%s:%s", host, port)

    MongoClient.connect(url, async function (err, db) {
        if (err) throw err;
        console.log("Database created!");
        var dbo = db.db("employeeDB");
        var employeeCollection = await dbo.createCollection("employee");

        if (employeeCollection.findOne({ userName: "NAV" }) == null) {
            var user1 = { _id: 1, userId: 1, userName: "NAV", email: "nav@gamil.com", reportingManagrId: "", photo: "", department: "Accounting", designation: "CEO", region: "USA", office: "US", isRegistered: 1, isActive: 1, lastModifiedBy: "system", lastModifiedDate: "2020-03-30" };
            var user2 = { _id: 2, userId: 2, userName: "Sudha", email: "sudha@gamil.com", reportingManagrId: "1", photo: "", department: "IT", designation: "CTO", region: "USA", office: "US", isRegistered: 1, isActive: 1, lastModifiedBy: "system", lastModifiedDate: "2020-03-30" };
            await employeeCollection.save(user1);
            await employeeCollection.save(user2);
        }

        var departmentCollection = await dbo.createCollection("department");
        await departmentCollection.save({ _id: 1, departmentId: 1, name: "Accounting", code: "Accounting" });
        await departmentCollection.save({ _id: 2, departmentId: 2, name: "IT", code: "IT" });
        await departmentCollection.save({ _id: 3, departmentId: 3, name: "Accounting", code: "FAS" });
        await departmentCollection.save({ _id: 4, departmentId: 4, name: "Accounting", code: "RTA" });
        await departmentCollection.save({ _id: 5, departmentId: 5, name: "IT", code: "IT-FAS" });
        await departmentCollection.save({ _id: 6, departmentId: 6, name: "IT", code: "IT-RTA" });

        var desCollection = await dbo.createCollection("designation");
        await desCollection.save({ _id: 1, designationId: 1, Name: "CEO" });
        await desCollection.save({ _id: 2, designationId: 2, Name: "CTO" });
        await desCollection.save({ _id: 3, designationId: 3, Name: "VP" });
        await desCollection.save({ _id: 4, designationId: 4, Name: "AVP" });
        await desCollection.save({ _id: 5, designationId: 5, Name: "Sr. Manager" });
        await desCollection.save({ _id: 6, designationId: 6, Name: "Manager" });
        await desCollection.save({ _id: 7, designationId: 7, Name: "Asst.Manager" });
        await desCollection.save({ _id: 8, designationId: 8, Name: "Team Leader" });
        await desCollection.save({ _id: 9, designationId: 9, Name: "SSE" });
        await desCollection.save({ _id: 10, designationId: 10, Name: "Software Engineer" });
        await desCollection.save({ _id: 11, designationId: 11, Name: "Trainee" });

        await dbo.createCollection("adUser");

        var regCollection = await dbo.createCollection("region");
        await regCollection.save({ _id: 1, regionId: 1, Name: "IN" });
        await regCollection.save({ _id: 2, regionId: 2, Name: "USA" });

        db.close();

    });

});;